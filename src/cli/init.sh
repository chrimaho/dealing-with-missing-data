...
# UV Initialization
cd dealing-with-missing-data
curl -LsSf https://astral.sh/uv/install.sh | sh
uv self update
uv --version
uv python install=3.13 --link-mode=copy
uv init --python=3.13 --link-mode=copy --lib --name="dealing-with-missing-data" --description="It is common to have missing data points in any given data set. Here, we assess the different methods available."
